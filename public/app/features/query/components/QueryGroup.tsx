...
export class QueryGroup extends PureComponent<Props, State> {
  ...
  onChangeDataSource = async (newSettings: DataSourceInstanceSettings) => {
    ...
    const queries = this.updateQueries(newSettings, dsSettings);
  ...
  updateQueries = (newSettings: DataSourceInstanceSettings, dsSettings?: DataSourceInstanceSettings): DataQuery[] => {
    const { queries } = this.state;

    if (!newSettings.meta.mixed && dsSettings?.meta.mixed) {
      return queries.map((q) => {
        if (q.datasource !== expressionDatasource.name) {
